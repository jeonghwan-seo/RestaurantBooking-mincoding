...
from communication import TestableSmsSender, TestableMailSender
...
class SundayBookingScheduler(BookingScheduler):
    ...
    def get_now(self):
        result = datetime.strptime("2021/03/28 17:00", "%Y/%m/%d %H:%M")
        return result
...
class MondayBookingScheduler(BookingScheduler):
    ...
    def get_now(self):
        result = datetime.strptime("2024/06/03 17:00", "%Y/%m/%d %H:%M")
        return result
...
class TestableBookingScheduler(BookingScheduler):
    ...
    def get_now(self):
        format = "%Y/%m/%d %H:%M"
        return datetime.strptime(self.__date_time, format)
